export default function CalculateTree({datum, data_stash, card_dim, labels}) {
  const sx = card_dim.w+40, y = card_dim.h+50,
    lbls = labels || {}
  datum = datum ? datum : {id: "0", data: {fn: "FN", ln: "LN", gender: "M"}, rels: {spouses: [], children: []}}
  console.log('calculate tree', datum);
  let data = [
    {x: 0, y: 0, data: datum},
  ...
  if (datum.rels.spouses && datum.rels.spouses.length > 0) {
    data.push({x: -100, y: y, data: {rel_type: 'son', data: {label: lbls.son || "Add son", gender: "M"}}});
    data.push( {x: 100, y: y, data: {rel_type: 'daughter', data: {label: lbls.daughter || "Add daughter", gender: "F"}}})
  };
  data.filter(d => shouldAddRel(d.data.rel_type))
  // const data = [
  //   {x: 0, y: 0, data: datum},
  //   {x: -100, y: -y, data: {rel_type: 'father', data: {label: lbls.father || "Add father", gender: "M"}}},
  //   {x: 100, y: -y, data: {rel_type: 'mother', data: {label: lbls.mother || "Add mother", gender: "F"}}},
